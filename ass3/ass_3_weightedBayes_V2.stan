...
parameters {
  ...
  real logit_w1; // We estimate the weight parameters on the logit scale
  real logit_w2;
}

transformed parameters{
  real<lower=0, upper=1> weight1; // We use the weight parameters on the scale that makes sense in the model
  real<lower=0, upper=1> weight2;

  
  weight1 = inv_logit(logit_w1);
  weight2 = inv_logit(logit_w2);
  
}

// The model to be estimated.
model {
  // Priors for weights are set on the logodds transformed weights. Then we can use normal :)
  target += normal_lpdf(logit_w1 | 0, 1); // Setting sd too high here will lead to a bimodal distr when transforming back
  target += normal_lpdf(logit_w2 | 0, 1);
  ...
  target += normal_lpdf(to_vector(L_SecondRating) | bias + to_vector(L_FirstRating) * weight1 + to_vector(L_GroupRating) * weight2, error);
}
...
generated quantities {
  ...
  logit_w1_prior = normal_rng(0,1); // Generate the priors for weights as they were sampled
  logit_w2_prior = normal_rng(0,1);
  
  w1_prior = 0.5 + inv_logit(logit_w1_prior) / 2.0; // Transform back to the theretically sensible scale
  w2_prior = 0.5 + inv_logit(logit_w2_prior) / 2.0;
  
  w1_posterior = 0.5 + inv_logit(logit_w1) / 2.0;
  w2_posterior = 0.5 + inv_logit(logit_w2) / 2.0;
  
  
  
  // Saving the log_likelihood. Same as the likelihood function in the model chunk.
  for (t in 1:n){
    log_lik[t] = normal_lpdf(L_SecondRating[t] | bias + L_FirstRating[t] * weight1 + L_GroupRating[t] * weight2, error);
  }
